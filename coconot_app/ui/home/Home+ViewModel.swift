...
extension HomeView {
    ...
    class ViewModel {
        ...
        func fetchAllHotHouses(){
            Task {
                do {
                    let data = try await globalRepository.getAllHotHousesWithPredictions()
                    self.hotHousesWithPredictionsList = data
                } catch {
                    print("Erreur lors de la recupération des hotHouses avec predictions : \(error)")
                    self.hotHousesWithPredictionsList = []
                }
            }
        }
